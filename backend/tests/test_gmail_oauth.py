...
def test_build_authorization_url_includes_offline_access_and_signed_state(monkeypatch):
    ...
    auth_url = gmail_service.build_authorization_url(user_id=123)
    ...
    assert gmail_service.parse_oauth_state(params["state"][0]) == 123


def test_exchange_code_encrypts_tokens_and_stores_account(db_session, monkeypatch):
    setup_gmail_config(monkeypatch)
    state = gmail_service.create_oauth_state(user_id=7)
    ...
    row = gmail_service.exchange_code_for_tokens(db_session, "auth-code", state)
